...
ggplot(df, aes(x = Sleep.Duration, fill = Gender)) +
  ...
  facet_wrap(~Gender) +
  theme_minimal() +
  labs(title = "Histogram of Sleep Duration by Gender",
       x = "Sleep Duration (hours)",
       y = "Count")

```

From the histograms, sleep duration does not appear perfectly normally distributed within each gender. However, to assess normality more formally, we will examine **QQ plots** for both women and men.
...
ggplot(df, aes(sample = Sleep.Duration)) +
  ...
  facet_wrap(~Gender) +
  theme_minimal() +
  labs(title = "QQ Plot of Sleep Duration by Gender")

```

The QQ plots suggest that sleep duration does not follow a normal distribution for either gender, as points show substantial deviation from the reference line. To formally assess normality, the **Lilliefors test** was applied separately for women and men.
